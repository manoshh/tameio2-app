...
const ToastAction = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-stone-200 bg-transparent px-3 text-sm font-medium transition-colors hover:bg-stone-100 focus:outline-none focus:ring-2 focus:ring-stone-400 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-white/40 group-[.destructive]:hover:bg-red-700",
      className
    )}
    {...props}
  />
...
const ToastClose = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      // Σκόπιμα πάντα ορατό: το προηγούμενο ήταν opacity-0 με group-hover, οπότε
      // σε οθόνη αφής (χωρίς hover) το κουμπί ήταν αόρατο.
      "absolute right-2 top-2 rounded-md p-1 text-stone-400 transition-colors hover:text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-400 group-[.destructive]:text-red-100 group-[.destructive]:hover:text-white group-[.destructive]:focus:ring-red-200",
      className
    )}
    toast-close=""
    {...props}
