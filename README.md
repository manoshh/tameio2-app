...
npm run lint
npm run build
npm run test:e2e   # ⚠ μόνο σε άδεια βάση — δες παρακάτω
```

Το `test:e2e` καλεί τους API handlers πάνω σε **πραγματική** βάση: ελέγχει auth,
δικαιώματα, SQL injection, τύπους, ζώνες ώρας και rollback.

Επειδή γράφει και σβήνει δεδομένα, ματαιώνεται αυτόματα αν η βάση περιέχει έστω
μία πραγματική εγγραφή. Για να το τρέξεις με δεδομένα στην παραγωγή, φτιάξε ένα
Neon branch και στρέψε εκεί το `DATABASE_URL`.
